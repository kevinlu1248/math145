...
Proof.
  ...
  replace (0 + a + 1) with (1 + a) by ring.
  apply N3, N4.
Qed.
...
Proof.
  ...
  - induction n using A7P2.
    case (A2P3 n 1) => [? | [-> | /(A7P3 (-1))]];
                         first by firstorder using A3P4.
    + exists (a :: nil).
...
Theorem pos_lt: ∀ a, 0 < a ↔ positive a.
Proof.
  intros.
  split.
  - apply (induction, λ n, 0 < n → positive n); eauto using one_pos, add_pos.
  - intro. induction H. 
    apply N4. apply (trans_order _ n _); eauto using A7P1.
Qed.

Lemma pow_pos_add : ∀ a n, 0 < n → a ^ (n + 1) = a * a ^ n.
Proof.
  intros.
  rewrite (pow_pos_r a (n + 1)).
  apply (trans_order _ n _); eauto using A7P1.
  by replace (n + 1 - 1) with n by ring.
Qed.
...
Proof.
  ...
  - apply pos_lt in H0.
    destruct H.
    + induction H0.
      * rewrite pow_pos_add; auto.
        by rewrite pow_1_r M1.
      * 
  - subst. 
    rewrite pow_0_r.
    by rewrite M3 A3.
